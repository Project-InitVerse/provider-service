...
type InventoryRequestLister interface {
	...
	List(selector labels.Selector) (ret []*v1.InventoryRequest, err error)
	// Get retrieves the InventoryRequest from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1.InventoryRequest, error)
...
// List lists all InventoryRequests in the indexer.
func (s *inventoryRequestLister) List(selector labels.Selector) (ret []*v1.InventoryRequest, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.InventoryRequest))
	})
	return ret, err
}

// Get retrieves the InventoryRequest from the index for a given name.
func (s *inventoryRequestLister) Get(name string) (*v1.InventoryRequest, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("inventoryrequest"), name)
	}
	return obj.(*v1.InventoryRequest), nil
}
