...
import (
	...
	clusterClient "providerService/src/cluster/kube"
	"providerService/src/cluster/kube/metallb"
	"providerService/src/cluster/types/v1beta2"
	ctypes "providerService/src/cluster/types/v1beta2"
	clusterutil "providerService/src/cluster/util"
	providerflags "providerService/src/cmd/provider-services/cmd/flags"

	ipoptypes "providerService/src/operator/ipoperator/types"
	"providerService/src/operator/operatorcommon"
)
...
func doIPOperator(cmd *cobra.Command) error {
	configPath := viper.GetString(providerflags.FlagKubeConfig)
	ns := viper.GetString(providerflags.FlagK8sManifestNS)
	listenAddr := viper.GetString(providerflags.FlagListenAddress)
