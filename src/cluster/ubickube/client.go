...
import (
	...
	"providerService/src/cluster/ubickube/clientcommon"
	kubeclienterrors "providerService/src/cluster/ubickube/errors"
	crd "providerService/src/ubicpkg/api/ubicnet/v1"
	ubicclient "providerService/src/ubicpkg/client/clientset/versioned"
)
...
func NewClient(ctx context.Context, log log.Logger, ns string, configPath string) (cluster.UbicClient, error) {
...
func (c *client) GetManifestGroup(ctx context.Context, lID ctypes.LeaseID) (bool, crd.ManifestGroup, error) {
	leaseNamespace := builder.LidNS(lID)

	obj, err := c.ac.UbicnetV1().Manifests(c.ns).Get(ctx, leaseNamespace, metav1.GetOptions{})
	if err != nil {
...
func (c *client) TeardownLease(ctx context.Context, lid ctypes.LeaseID) error {
	...
	err := c.ac.UbicnetV1().Manifests(c.ns).Delete(ctx, builder.LidNS(lid), metav1.DeleteOptions{})
	if err != nil {
...
func (c *client) LeaseEvents(ctx context.Context, lid ctypes.LeaseID, services string, follow bool) (ctypes.EventsWatcher, error) {
...
func (c *client) LeaseLogs(ctx context.Context, lid ctypes.LeaseID,
	...
	for i, pod := range pods.Items {
		...
		streams[i] = cluster.NewUbicServiceLog(pod.Name, stream)
	}
	return streams, nil
}

func (c *client) ForwardedPortStatus(ctx context.Context, leaseID ctypes.LeaseID) (map[string][]ctypes.ForwardedPortStatus, error) {
	settingsI := ctx.Value(builder.SettingsKey)
	if nil == settingsI {
...
func (c *client) LeaseStatus(ctx context.Context, lid ctypes.LeaseID) (map[string]*ctypes.ServiceStatus, error) {
	settingsI := ctx.Value(builder.SettingsKey)
	if nil == settingsI {
		return nil, kubeclienterrors.ErrNotConfiguredWithSettings
	}
	...
	phResult, err := c.ac.UbicnetV1().ProviderHosts(c.ns).List(ctx, metav1.ListOptions{
...
func (c *client) ServiceStatus(ctx context.Context, lid ctypes.LeaseID, name string) (*ctypes.ServiceStatus, error) {
	if err := c.leaseExists(ctx, lid); err != nil {
		return nil, err
	}
	...
	mani, err := c.ac.UbicnetV1().Manifests(c.ns).Get(ctx, builder.LidNS(lid), metav1.GetOptions{})
	if err != nil {
...
exposeCheckLoop:
	...
	if hasHostnames {
		...
		phs, err := c.ac.UbicnetV1().ProviderHosts(c.ns).List(ctx, metav1.ListOptions{
			LabelSelector: labelSelector.String(),
		})
...
func (c *client) leaseExists(ctx context.Context, lid ctypes.LeaseID) error {
...
func (c *client) deploymentsForLease(ctx context.Context, lid ctypes.LeaseID) (map[string]*ctypes.ServiceStatus, error) {
	if err := c.leaseExists(ctx, lid); err != nil {
		return nil, err
	}
