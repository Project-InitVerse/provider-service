...
// IgnoreListReadOnly is interface
type IgnoreListReadOnly interface {
	IsFlagged(id ctypes.LeaseID) bool
	Size() int
}

// IgnoreList is interface
type IgnoreList interface {
	IgnoreListReadOnly
	Prepare(pd PreparedResult) error
	AddError(id ctypes.LeaseID, err error, extra ...string)
	Prune() bool
}
...
type ignoreList struct {
	entries map[ctypes.LeaseID]ignoreListEntry
	cfg     IgnoreListConfig
}

// NewIgnoreList is function to create Ignore list
func NewIgnoreList(config IgnoreListConfig) IgnoreList {
	return &ignoreList{
		entries: make(map[ctypes.LeaseID]ignoreListEntry),
		cfg:     config,
	}
}
...
func (il *ignoreList) Prepare(pd PreparedResult) error {
	...
	err := il.each(func(leaseID ctypes.LeaseID, lastError error, failedAt time.Time, count uint, extra ...string) error {
...
func (il *ignoreList) each(f func(k ctypes.LeaseID, failure error, failedAt time.Time, count uint, extra ...string) error) error {
...
func (il *ignoreList) AddError(k ctypes.LeaseID, failure error, extra ...string) {
...
func (il *ignoreList) IsFlagged(k ctypes.LeaseID) bool {
