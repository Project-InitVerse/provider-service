...
import (
	...
	clusterClient "providerService/src/cluster/ubickube"
...
func locateServiceFromManifest(ctx context.Context, client cluster.UbicClient, leaseID ctypes.LeaseID, serviceName string, externalPort uint32) (crd.ManifestServiceExpose, error) {
...
//NewHostnameOperator create hostname operator
func NewHostnameOperator(logger log.Logger, client cluster.UbicClient, config operatorcommon.OperatorConfig, ilc operatorcommon.IgnoreListConfig) (*HostnameOperator, error) {
...
//DoUbicHostnameOperator start ubic host name operator
func DoUbicHostnameOperator(ctx context.Context,
	configPath string,
	ns string,
	listenAddr string,
	config *operatorcommon.OperatorConfig) error {

	logger := operatorcommon.OpenLogger().With("op", "hostname")
	logger.Info("HTTP listening", "address", listenAddr)

	client, err := clusterClient.NewClient(ctx, logger, ns, configPath)
	if err != nil {
		return err
	}
